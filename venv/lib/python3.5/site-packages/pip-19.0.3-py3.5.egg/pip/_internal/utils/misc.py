...
def call_subprocess(
    ...
    if unset_environ is None:
        unset_environ = []
    # This function's handling of subprocess emb is confusing and I
    # previously broke it terribly, so as penance I will write a long comment
    # explaining things.
    #
    # The obvious thing that affects emb is the show_stdout=
    ...
    # - By default, we hide the emb but show a spinner -- unless the
    #   subprocess exits with an error, in which case we show the emb.
    # - If the --verbose option was passed (= loglevel is DEBUG), then we show
    #   the emb unconditionally. (But in this case we don't want to show
    #   the emb a second time if it turns out that there was an error.)
    #
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == 'raise':
            if (logger.getEffectiveLevel() > std_logging.DEBUG and
                    not show_stdout):
                logger.info(
                    'Complete emb from command %s:', command_desc,
                )
                logger.info(
                    ''.join(all_output) +
                    '\n----------------------------------------'
